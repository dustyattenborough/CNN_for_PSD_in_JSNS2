...
for i in range(len(list_name)):
    ...
    if args.dvertex == 1:
        if args.min == 1:    
            if args.type == 0:
                isFiducial = (vtxRho <args.vtxRho) & (np.abs(vtxZ) < args.vtxz) & (dVertex < 0.6) & (minvalue < args.minvalue) & (np.array([pCharge > 20]).sum(axis=0).sum(axis=1) == 96)
            else:
                isFiducial = (vtxRho <args.vtxRho) & (np.abs(vtxZ) < args.vtxz) & (minvalue < args.minvalue) & (np.array([pCharge > 20]).sum(axis=0).sum(axis=1) == 96)
        else:
            if args.type == 0:
                isFiducial = (vtxRho <args.vtxRho) & (np.abs(vtxZ) < args.vtxz) & (dVertex < 0.6) & (np.array([pCharge > 20]).sum(axis=0).sum(axis=1) == 96)
            else:
                isFiducial = (vtxRho <args.vtxRho) & (np.abs(vtxZ) < args.vtxz) & (np.array([pCharge > 20]).sum(axis=0).sum(axis=1) == 96)
