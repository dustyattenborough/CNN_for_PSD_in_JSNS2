...
for i in range(len(list_name)):
    ...
    if args.dvertex == 1:
        if args.min == 1:    
            if args.type == 0:
                isFiducial = (vtxRho <args.vtxRho) & (vtxZ < args.vtxz + 0.4) & (vtxZ > args.vtxz) & (dVertex < 0.6) & (minvalue < args.minvalue)  
            else:
                isFiducial = (vtxRho <args.vtxRho) & (vtxZ < args.vtxz + 0.4) & (vtxZ > args.vtxz) & (minvalue < args.minvalue)
        else:
            if args.type == 0:
                isFiducial = (vtxRho <args.vtxRho) & (vtxZ < args.vtxz + 0.4) & (vtxZ > args.vtxz) & (dVertex < 0.6) 
            else:
                isFiducial = (vtxRho <args.vtxRho) & (vtxZ < args.vtxz + 0.4) & (vtxZ > args.vtxz)
