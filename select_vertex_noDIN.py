...
for i in range(len(list_name)):
    ...
    if args.cut == 1:
        ...
        if args.dvertex == 1:
            if args.min == 1:    
                if args.type == 0:
                    isFiducial = (vtxRho >1.4) & (vtxRho <1.6) & (np.abs(vtxZ) < args.vtxz) & (dVertex < 0.6) & (minvalue < args.minvalue)
                else:
                    isFiducial = (vtxRho >1.4) & (vtxRho <1.6) & (np.abs(vtxZ) < args.vtxz) & (minvalue < args.minvalue)
            else:
                if args.type == 0:
                    isFiducial = (vtxRho >1.4) & (vtxRho <1.6) & (np.abs(vtxZ) < args.vtxz) & (dVertex < 0.6)
                else:
                    isFiducial =(vtxRho >1.4) & (vtxRho <1.6) & (np.abs(vtxZ) < args.vtxz)
